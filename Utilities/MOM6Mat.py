...
class CovMOM6CCS(CovMOM6):
	...
	def __init__(self,*args,depth_idx=0,**kwargs):
		if depth_idx<self.chl_depth_idx:
			variable_list = VariableList(['thetao','so','no3','chl','o2','ph'])
		else:
			variable_list = VariableList(['thetao','so','no3','o2','ph'])
	...
	for covclass in [CovMOM6CCS]:
		for depth in [2,4,6,8,10,12,14,16]:
			...
			try:
				...
				dummy.calculate_cov()
				dummy.scale_cov()
			idummy = InverseGOM(depth_idx = depth)
			datascale = load(idummy.make_datascale_filename())
