...
ss_e_vals,shallow_ss_e_vecs = scipy.sparse.linalg.eigs(cov_holder_ph,k=2)

cov_holder_ph = CovElement.load(deep_dummy.trans_geo, "ph", "ph")
ss_e_vals,deep_ss_e_vecs = scipy.sparse.linalg.eigs(cov_holder_ph,k=2)
...
ss_plot = cov_holder_ph.trans_geo.transition_vector_to_plottable(shallow_ss_e_vecs[:, 1])
...
ss_plot = cov_holder_ph.trans_geo.transition_vector_to_plottable(deep_ss_e_vecs[:, 1])
...
XX,YY = cov_holder_ph.trans_geo.get_coords()
pcm = ax1.pcolor(XX,YY,ss_plot,vmin=vmin,vmax=vmax,cmap='RdYlBu')
