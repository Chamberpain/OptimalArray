...
total_obs = [trans_mat.multiply(x,value=0.00001) for x in [8]]
float_list = [Core.recent_floats(trans_mat.trans_geo, ArgoReader,days_delta=(90*x)) for x in [8]]
...
for trans,float_mat,label,k in zip(total_obs,float_list,[''],[8]):

	ax = fig.add_subplot(1,1,i, projection=ccrs.PlateCarree())
	ax_list.append(ax)
	...
	XX,YY,ax = trans_mat.trans_geo.plot_setup(ax=ax,adjustable=True)
	...
	for idx in scipy.sparse.find(float_mat.get_sensor(var))[0]:
		...
		lc = colorline(lons,lats,ax,z = [90*x for x in range(k)],norm=plt.Normalize(0.0, 720.0),linewidth=3)
	i += 1
fig.colorbar(lc,ax=ax_list,label='Days Since Deployment',location='top')
fig.colorbar(pcm,ax=ax_list,pad=.05,label='Chance of Float (%)',location='bottom')
plt.savefig(file_handler.out_file('figure_4'),bbox_inches='tight')
plt.close()